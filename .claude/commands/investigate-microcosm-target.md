---
description: Investigate a Microcosm calibration target discrepancy with specialist agents
argument-hint: "[--release RELEASE_ID] TARGET_ID"
---

# Investigate Microcosm Target

Investigate a discrepant Microcosm calibration target using the Claude Code harness, not the dashboard UI.
...
```bash
node scripts/microcosm-investigation-packet.mjs $ARGUMENTS --out investigations/latest-target-packet.json
```
...
Input:
- full packet JSON
- chronicle dimensions and filters
- artifact warnings
...
Input:
- full packet JSON
- source/model variable names from packet and chronicle

Task:
- Inspect `policyengine-us`, `policyengine-core`, and any relevant model repos.
- Verify variable definition, entity, period, unit, sign convention, and whether it matches the chronicle measure.
...
## Verdict
One of:
- confirmed chronicle/source target issue
