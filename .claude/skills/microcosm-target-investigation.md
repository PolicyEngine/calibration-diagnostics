# Microcosm Target Investigation Skill

Use this skill when investigating why a Microcosm calibration target has a bad fit, odd target value, suspicious estimate, or unexpected version-over-version movement.
...
```bash
node scripts/microcosm-investigation-packet.mjs --release <release-id> <target-id> --out investigations/latest-target-packet.json
```

The packet includes:
- target row and estimates,
- Chronicle fact fields,
- source metadata,
- artifact paths,
- repo search links,
...
Evidence should come from Populus, Arch, source extractors, or chronicle construction code.

### 2. Target Materialization

Verify:
- exact chronicle filters compile into a model selector,
