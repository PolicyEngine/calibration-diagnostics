...
def write_bundle(path: Path, rows: list[dict], *, with_manifest: bool = True) -> Path:
    ...
    if with_manifest:
        (path / "manifest.json").write_text(
            json.dumps(
                {
                    # Deprecated upstream identifiers: this fixture verifies
                    # compatibility with older Chronicle consumer manifests.
                    ...
                    "ledger_commit": "chronicle-commit-1",
                    "ledger_release": "chronicle-release-1",
                }
            )
        )
    return path
