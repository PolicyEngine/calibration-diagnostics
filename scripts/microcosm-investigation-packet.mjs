...
const DEFAULT_BASE_URL = process.env.MICROCOSM_DIAGNOSTICS_BASE_URL ?? PRODUCTION_BASE_URL;
...
function usage() {
  console.error(
    [
      "Usage:",
      "  node scripts/microcosm-investigation-packet.mjs [--release RELEASE_ID] TARGET_ID [--out FILE] [--base-url URL]",
      "",
      "Examples:",
      "  node scripts/microcosm-investigation-packet.mjs irs_soi.ty2022.historic_table_2.us.under_1.ctc_amount",
      "  node scripts/microcosm-investigation-packet.mjs --release microcosm-us-2024-incumbent-improved-996401a-20260618 TARGET --out investigations/target.json",
    ].join("\n"),
  );
}
...
function investigationUrl(baseUrl, release, target) {
  const url = new URL("/api/microcosm/target-investigation", baseUrl);
...
async function main() {
  ...
  const candidates = explicitBaseUrl || process.env.MICROCOSM_DIAGNOSTICS_BASE_URL
    ? [baseUrl]
