...
// anchored to the Microcosm palette so the map reads as part of the same
// design system — on-target cells carry the identity's verdigris, the far-off
// end lands on the functional error red, and unscored cells use the neutral
// green-gray. The warm midpoints keep the surface reading as a calm good → bad
// heat map rather than neon traffic lights. Anchors are pre-resolved to RGB
// because the ramp interpolates numerically; each stop names the token it
// draws from.
...
const STOPS: Stop[] = [
  { at: 0.0, rgb: [62, 122, 94] }, // --accent    #3E7A5E verdigris (on target)
  { at: 0.03, rgb: [85, 145, 115] }, // lighter verdigris step
...
const MISSING: [number, number, number] = [110, 127, 116]; // --text-tertiary #6E7F74 — no score
