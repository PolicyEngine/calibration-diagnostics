...
const targets: NationalTarget[] = [
  { name: "agi_total", target: 1000, microcosm: 1100, source: "irs_soi", variable: "agi", measure: "total" },
  { name: "agi_count", target: 100, microcosm: 90, source: "irs_soi", variable: "agi", measure: "count" },
  { name: "eitc_total", target: 200, microcosm: 260, source: "irs_soi", variable: "eitc", measure: "total" },
  { name: "snap_total", target: 500, microcosm: 480, source: "usda_snap", variable: "snap", measure: "total" },
  { name: "zero_bench", target: 0, microcosm: 5, source: "irs_soi", variable: "x", measure: "total" },
];
...
test("microcosm loss is capped-MAPE over targets with a nonzero official value", () => {
  const microcosm: DatasetInput = { label: "microcosm", value: (t) => t.microcosm };
  const s = scoreDataset(targets, microcosm);
...
test("concept breakdown groups by variable × measure and scores each dataset", () => {
  ...
  const groups = conceptBreakdown(targets, [microcosm, taxcalc]);
  ...
  expect(agiTotal.scores.microcosm.covered).toBe(1);
