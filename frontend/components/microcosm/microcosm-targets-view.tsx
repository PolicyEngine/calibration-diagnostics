...
interface Column {
  ...
  numeric?: boolean;
  sortable?: boolean;
  render: (row: MicrocosmTargetRow) => React.ReactNode;
}

interface VariableMeasureOption {
  key: string;
  label: string;
  row: MicrocosmVariableRow;
}
...
function finalError(row: MicrocosmTargetRow) {
...
function dimensionSummary(row: MicrocosmTargetRow): string {
  const dims = row.target_dimensions?.map((dim) => dim.value).filter(Boolean) ?? [];
  if (dims.length) return dims.join(" · ");
  const group = row.chronicle?.layout_groupby_value_id;
...
const OVERVIEW_COLUMNS: Column[] = [
  {
    ...
    sortable: true,
    render: (row) => (
      <div className="max-w-sm" title={row.chronicle?.measure_concept ?? String(row.name ?? "")}>
        <div className="font-medium text-foreground">{measureTitle(row)}</div>
        <div className="truncate text-xs text-muted-foreground">
          {row.chronicle?.measure_unit
            ? row.chronicle.measure_unit.toUpperCase()
            : row.measure === "total"
              ? "Amount"
              : row.measure === "count"
                ? "Count"
  ...
  },
  {
    ...
    sortable: true,
    render: (row) => (
      <span className="whitespace-nowrap" title={row.chronicle?.geography_id ?? undefined}>
        {row.geography || "—"}
      </span>
    ),
  },
...
function dimensionColumns(dimensions: MicrocosmTargetDimension[]): Column[] {
  return dimensions.map((dim) => ({
    key: dim.key,
    label: dim.label,
    sortable: true,
    render: (row: MicrocosmTargetRow) => {
...
function groupVariables(variables: MicrocosmVariableRow[]): VariableGroup[] {
  const groups = new Map<string, MicrocosmVariableRow[]>();
...
function healthcareSummary(variables: MicrocosmVariableRow[]): HealthcareSummary {
  let nTargets = 0;
  let within10Pct = 0;
