...
import {
  ...
  type MicrocosmTreemapGroup,
  type MicrocosmTreemapLeaf,
...
function valueFmt(row: MicrocosmTargetRow, value: number | null | undefined): string {
  return row.chronicle?.measure_unit?.toLowerCase() === "usd"
    ? fmtMoney(value)
    : fmtCompact(value);
}
...
function rowBreakdown(row: MicrocosmTargetRow): string {
  const dims =
...
function errorText(row: MicrocosmTargetRow): string {
...
export function ClusterDetail({
  ...
  function fullDiagnosticsPath(): string {
    if (synthetic && leaf.source === "__other_sources__") return "/microcosm/targets";
