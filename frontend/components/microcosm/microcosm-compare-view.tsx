...
function errorValue(row: MicrocosmComparisonRow, side: "a" | "b") {
  const value = side === "a" ? row.a_error : row.b_error;
  if (row.error_kind === "absolute") return fmtCompact(value);
  return relErr(value);
}

function targetLabel(row: MicrocosmComparisonRow) {
...
function comparisonDimensionLabel(row: MicrocosmComparisonRow): string {
  const parts =
    row.breakdown
      ?.split(" · ")
      .map((part) => part.trim())
...
function targetSearchText(row: MicrocosmComparisonRow) {
  return [
    row.name,
    row.source,
    row.variable_key,
...
function dimensionValue(row: MicrocosmComparisonRow, label: string): string | null {
  const dim = row.target_dimensions?.find((entry) => entry.label === label);
  if (dim?.value) return dim.value;
  return null;
}

function dimensionFilters(rows: MicrocosmComparisonRow[]) {
...
function compareSortValue(row: MicrocosmComparisonRow, key: string): string | number | null {
  switch (key) {
    case "source":
...
function comparisonTargetValue(row: MicrocosmComparisonRow) {
...
function isBoundedRelativeMover(row: MicrocosmComparisonRow) {
  return (
...
function MoverList({ rows }: { rows: MicrocosmComparisonRow[] }) {
...
function TargetComparisonTable({ rows }: { rows: MicrocosmComparisonRow[] }) {
...
export function MicrocosmCompareView() {
  const { data: releaseData, isLoading: releasesLoading } = useMicrocosmReleases();
  ...
  const { data, isLoading, error } = useMicrocosmCompare(a, b);
