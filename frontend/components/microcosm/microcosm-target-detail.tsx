      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/70 px-5 py-3">
        <div className="min-w-0">
          ...
          <code className="mt-1 block break-all text-xs text-muted-foreground">
            {chronicle?.source_record_id ?? row.name}
          </code>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-4 px-5 py-4 lg:grid-cols-2">
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Unit" value={chronicle?.measure_unit?.toUpperCase()} />
            <Field label="Domain" value={titleFromIdentifier(chronicle?.domain)} />
          </div>
          {shownDims.length > 0 && (
            <div className="grid grid-cols-2 gap-3">
              {shownDims.map((dim) => (
          ...
          <div className="grid grid-cols-1 gap-3 border-t border-border/60 pt-3">
            <CodeField label="Fact key" value={chronicle?.fact_key} />
            <CodeField label="Source record ID" value={chronicle?.source_record_id} />
            <CodeField label="Measure concept" value={chronicle?.measure_concept} />
            <CodeField label="Source concept" value={chronicle?.source_concept} />
            <CodeField label="Record set" value={chronicle?.layout_record_set_id} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <CodeField label="Group-by dimension" value={chronicle?.layout_groupby_dimension} />
            <CodeField label="Group-by value" value={chronicle?.layout_groupby_value_id} />
            <CodeField label="Layout measure" value={chronicle?.layout_measure_id} />
            <CodeField label="Geography ID" value={chronicle?.geography_id} />
          </div>
          {chronicle?.filters?.length ? (
            <div className="border-t border-border/60 pt-3">
              <div className="mb-2 text-[11px] uppercase tracking-wider text-muted-foreground">
                Filters
              </div>
              <div className="grid gap-2">
                {chronicle.filters.map((filter) => (
                  <div
                    key={filter.key}
