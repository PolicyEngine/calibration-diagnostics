...
- [ ] Country and release are explicit.
...
- [ ] Manifest, index, tiers, target index, and detail shards use schema 3.
...
- [ ] `BLOB_READ_WRITE_TOKEN` is available only to the server-side publisher.
...
- [ ] Release switching cannot display another commit's data.
